                                    ...
                                    <table class="align-items-center mb-0 table table-borderless" id="user_list_table" style="width:100%">
                                        <thead class="thead-light">
                                            <tr>
                                                ...
                                                <th class="text-uppercase text-secondary text-center text-xxs font-weight-bolder opacity-7">Facturación</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php echo $tabla; ?>
...
<script>
    $(document).ready(function() {
        ...
        $(".btn_factura").on("click", function() {
            ...
            $.ajax({
                type: 'POST',
                url: '/ComprobantePago/generarXmlFacturacion_3_3',
                dataType: 'json',
                data: {clave, user_id},
                beforeSend: function() {
                    // setting a timeout
                ...
                success: function(data) {
                    console.log(data);
                    // swal("En un lapso de 48 horas recibirá su comprobante.", {
                    //     icon: 'success',
                    ...
                    // });
                    // // recargar();
                    // if (data.resp.uuid == "comprobante_pago") {
                    //     obtenerPdfComprobante(data.resp);
                    // } else {
                    //     obtenerPdfFacturacion(data.resp);
                    // }
        ...
        file_input.onchange = (evt) => {
            ...
            if (file) {
                ...
                if (sizekiloBytes > 5000) {
                    Swal.fire({
                        title: 'Alerta',
                        text: '¡El archivo seleccionado supera el tamaño permitido!',
                        icon: 'error'

                    })
                    $("#file_input").css({
                        "border-color": "red",
                        "box-shadow": "0 0 0 2px #ffa9a9"
                    });
                    $("#file_input").val('');
