...
sql;
  ...
  public static function getProductCartException($user_id, $id_producto1, $id_producto2)
  {
    $mysqli = Database::getInstance();
    $query = <<<sql
      SELECT * FROM carrito WHERE id_producto = $id_producto1 OR id_producto = $id_producto2  AND user_id = $user_id and status = 1
sql;
    return $mysqli->queryAll($query);
  }
  ...
  public static function insertProductCart($data)
  {
    $mysqli = Database::getInstance();
    $query = <<<sql
      INSERT INTO carrito(id_producto, grupo,user_id, status) VALUES ('$data->_id_producto','$data->_grupo','$data->_user_id',1)
sql;
...
sql;
  ...
  public static function getCarritoByIdUser($user_id)
  {
    $mysqli = Database::getInstance();
    $query = <<<sql
      SELECT c.id_carrito,pro.id_producto,pro.nombre,pro.precio_publico,pro.precio_socio,pro.tipo_moneda,pro.caratula,pro.es_curso,pro.es_servicio,pro.es_congreso,ua.monto_congreso as amout_due, ua.clave_socio
      FROM productos pro
...
sql;
  ...
  public static function getTicketUser($user_id, $clave)
  {
    $mysqli = Database::getInstance();
    $query = <<<sql
      SELECT pro.id_producto,pro.nombre,pro.precio_publico,pro.precio_socio,pro.tipo_moneda,pro.caratula,pro.es_curso,pro.es_servicio,pro.es_congreso,ua.monto_congreso as amout_due,ua.clave_socio, pp.status,pp.monto,pp.tipo_pago,pp.tipo_moneda as t_moneda, pp.clave
      FROM productos pro         
...
sql;
  ...
  public static function getTicketUserCompra($user_id, $clave)
  {
    $mysqli = Database::getInstance();
    $query = <<<sql
    SELECT pro.id_producto,pro.nombre,pro.precio_publico,pro.precio_socio,pro.tipo_moneda,pro.caratula,pro.es_curso,pro.es_servicio,pro.es_congreso,ua.monto_congreso as amout_due,ua.clave_socio, pp.status,pp.monto,pp.tipo_pago,pp.tipo_moneda as t_moneda, pp.clave, ap.fecha_asignacion
    FROM productos pro         
    INNER JOIN pendiente_pago pp ON (pp.id_producto = pro.id_producto)
    INNER JOIN utilerias_administradores ua ON(pp.user_id = ua.user_id)
    INNER JOIN asigna_producto ap ON (pp.id_producto = ap.id_producto and ap.user_id = pp.user_id)      
...
sql;
  ...
  public static function getProductsGroup($user_id,$grupo)
  {
    $mysqli = Database::getInstance();
    $query = <<<sql
      SELECT count(1) as total_grupo FROM carrito WHERE user_id = $user_id and grupo = $grupo and status = 1
sql;
    return $mysqli->queryAll($query);
  }
